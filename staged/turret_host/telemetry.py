...
**`attitude` in a control row, and why it duplicates imu.jsonl.** The row
carries the pitch gravity MEASURED beside `pose_deg[0]`, the pitch the step
count COMMANDED, and their ratio. Two of those three could be recovered by
joining imu.jsonl on time; the ratio could not, and the join is exactly where
an analysis goes wrong -- the two streams are sampled at different rates and
the IMU one drops samples under lock contention, so `age_ms` travels with the
row to say how far apart they really were. `k_row` is the number that settles
whether DIFFERENTIAL_N is right, and it is written per row rather than as a
summary because a median cannot be binned against pitch or rate.
...
class FlightRecorder:
    ...
    def log_control(self, *, t, state, est, out, laser, interlock, link,
                    pose=None, attitude=None, frame_t=None, frame_index=None,
        ...
        try:
            ...
            if out is not None:
                rec["cmd"] = {
                    ...
                    # Which tier fed the feedforward, and how much evidence was
                    # behind it. Without this a run cannot distinguish lag from
                    # pure-P frames from lag in the control law itself.
                    "ff_inertial_u": _f(getattr(out, "ff_inertial_u", None)),
                    "ff_inertial_v": _f(getattr(out, "ff_inertial_v", None)),
                    "ff_source": getattr(out, "ff_source", "est"),
                    "ff_points": int(getattr(out, "ff_points", 0) or 0),
            ...
            if attitude is not None:
                # THE POSE OVERLAY'S TWO NUMBERS, and the ratio between them.
                # `pose_deg[0]` above is the COMMANDED pitch, dead-reckoned
                # from the step count; `pitch_deg` here is what gravity
                # actually measured. Their ratio is degrees of real tilt per
                # commanded degree -- the one-way, large-angle measurement that
                # is not blind to a scale error, and therefore the only check
                # on DIFFERENTIAL_N that does not come from the same arithmetic
                # it is meant to test.
                #
                # BOTH the per-row ratio and the running median are written,
                # and the per-row one is the point: a median cannot be binned.
                # `k_row` is None below POSE_SCALE_MIN_DEG because a ratio of
                # two small angles is dominated by the 0.65-2.3 deg of
                # backlash, which is exactly how every previous measurement of
                # this number (1.171, 1.072, 0.972) went wrong.
                cmd = None if pose is None else pose[0]
                k_row = None
                if (attitude.ok and not attitude.moving and cmd is not None
                        and abs(cmd) >= config.POSE_SCALE_MIN_DEG
                        and abs(attitude.pitch_deg) >= config.POSE_SCALE_MIN_DEG):
                    k_row = abs(attitude.pitch_deg) / abs(cmd)
                rec["attitude"] = {
                    "ok": bool(attitude.ok),
                    "pitch_deg": _f(attitude.pitch_deg),      # measured, gravity
                    "cmd_pitch_deg": _f(cmd),                 # the one k used
                    "roll_deg": _f(attitude.roll_deg),
                    # Sample age. The measured and commanded pitch are NOT
                    # simultaneous: the IMU is sampled at 10 Hz and dropped
                    # under lock contention, so a row with a large age is a row
                    # where some of the gap is lag, not mechanism.
                    "age_ms": _f(attitude.age_s * 1000.0),
                    "moving": bool(attitude.moving),
                    "rate_dps": _f(attitude.rate_dps),
                    "k_row": _f(k_row),
                    "k_median": _f(attitude.scale),
                }
