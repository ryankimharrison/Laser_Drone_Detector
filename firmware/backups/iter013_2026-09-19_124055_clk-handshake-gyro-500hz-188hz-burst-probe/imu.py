...
_ITG_DLPF_FS = 0x16
_ITG_INT_CFG = 0x17
_ITG_INT_STATUS = 0x1A
_ITG_TEMP_OUT_H = 0x1B
_ITG_PWR_MGM = 0x3E
_ITG_GYRO_XOUT_H = 0x1D

#: Bytes from TEMP_OUT_H through GYRO_ZOUT_L: temperature then all three axes,
#: in ONE burst. The datasheet permits this -- the address pointer
#: auto-increments and only stops at the reserved block above register 33
#: (0x22 = GYRO_ZOUT_L is the last register below it), so 0x1B + 8 lands
#: exactly on the boundary. Temperature is worth the two extra bytes: gyro bias
#: drifts with it, and integrating a gyro means that drift becomes angle.
_ITG_BURST_LEN = 8

#: INT_STATUS bit 0, RAW_DATA_RDY: set when a new sample has landed in the
#: data registers. THE ITG-3205 HAS NO FIFO -- the register map is 0x00,
#: 0x15-0x17, 0x1A-0x22, 0x3E and nothing else. ("FIFO" appears once in the
#: whole datasheet, as an unexplained label in the block diagram, and is absent
#: from the list of functional blocks.) So a sample not read before the next
#: one lands is gone with no way to know afterwards, which for an INTEGRATED
#: signal is a permanent angle error that nothing downstream can detect. This
#: bit is the only witness.
_ITG_RAW_DATA_RDY = 0x01
...
class GY85:
    def __init__(self, port=None, freq=None):
        ...
        # How many times the ADXL345 had to be kicked out of standby this
        # session. Non-zero is not a failure, but a climbing count is a part
        # on its way out -- accel_live() reports it in plain words.
        self.accel_recoveries = 0
    ...
    def begin(self):
        ...
        if g in found:
            ...
            time.sleep_ms(10)
            # FS_SEL = 3 (0x18): +/-2000 deg/s, the only scale this part has,
            # and the datasheet is explicit that 3 is the ONLY valid value --
            # "Set to 03h for proper operation". 0, 1 and 2 are reserved.
            #
            # DLPF_CFG = 1 (0x01): 188 Hz bandwidth, 1 kHz internal rate.
            # WAS 3 = 42 Hz, and that was too narrow. The gyro exists here to
            # tell the host how much of the image motion was the TURRET moving
            # rather than the drone, which means the interesting part of the
            # signal is precisely the transients at the start and stop of each
            # move -- and a 42 Hz filter smooths those away before the sample
            # is ever taken. Sampling faster through a narrow filter just
            # returns more copies of an already-smoothed signal.
            #
            # Noise is not the reason to keep it narrow. Datasheet: 0.7 deg/s
            # RMS at DLPF_CFG=2 (98 Hz), so ~0.97 deg/s at 188 Hz. Integrated
            # across one 33 ms camera frame at 500 Hz that is
            # 0.97*sqrt(0.002*0.0333) = 0.008 deg = 0.19 px at this image
            # scale, against a 25 px firing threshold. Buy bandwidth with it.
            #
            # Only DLPF_CFG=0 runs the 8 kHz internal rate; 1-6 are all 1 kHz,
            # so 1 kHz is the real output ceiling. 0 would give 256 Hz of
            # bandwidth against a 250 Hz Nyquist at 500 Hz output -- aliasing.
            # 188 Hz sits safely under it.
            self._w(g, _ITG_DLPF_FS, 0x18 | 0x01)
            self._w(g, _ITG_SMPLRT_DIV, 1)        # 1 kHz / (1+1) = 500 Hz
            # RAW_RDY_EN. Enabled for the STATUS BIT, not for the pin: it lets
            # a reader tell "no new sample yet" from "a sample I already have",
            # which is the only defence against silently re-reading or dropping
            # samples when the read loop and the sensor's own 500 Hz cadence
            # drift against each other. See _ITG_RAW_DATA_RDY above.
            self._w(g, _ITG_INT_CFG, _ITG_RAW_DATA_RDY)
            self.have_gyro = True
    ...
    def gyro_ready(self):
        """True if a new sample has landed since the last read of the data.

        Reading INT_STATUS clears it, so this is one-shot per sample. Cheap:
        a single byte.
        """
        return bool(self._r(config.IMU_ADDR_GYRO, _ITG_INT_STATUS, 1)[0]
                    & _ITG_RAW_DATA_RDY)

    def gyro_burst_into(self, buf):
        """Temperature + all three axes into a CALLER-OWNED 8-byte buffer.

        One I2C transaction, and no allocation in the caller's loop -- which
        is the whole point. At 400 kHz a burst of this length is about 250 us
        of bus time, so 500 Hz costs ~13% of the bus.

        Layout, big-endian signed 16-bit pairs:
            [0:2] temp   [2:4] gyro x   [4:6] gyro y   [6:8] gyro z
        """
        self.bus.readfrom_mem_into(config.IMU_ADDR_GYRO, _ITG_TEMP_OUT_H, buf)
    ...
    def _configure_accel(self):
        """Put the ADXL345 into measure mode, in the datasheet's order.

        Standby, configure, THEN measure. Writing BW_RATE/DATA_FORMAT while
        the part is already measuring is the documented way to leave it in an
        undefined state, and the original code here did exactly that.

        Separate from begin() because it is also the RECOVERY path -- see
        accel_live(). Must be safe to call repeatedly on a running part.
        """
        a = config.IMU_ADDR_ACCEL
    ...
    def accel_live(self, samples=16, wait_ms=5):
        """(ok, reason), reconfiguring once if the part has gone quiet.

        This part LATCHES INTO STANDBY during operation. Observed twice on
        this machine: it reads a healthy |a| ~ 0.96 g, then some minutes and
        one homing run later every sample is (0, 0, 0) -- while DEVID still
        reads 0xE5 and POWER_CTL still says measure mode. A Pico reset revives
        it, and a reset re-runs _configure_accel(), so re-sending that
        sequence is the actual cure rather than the reset.

        Recovering here rather than making the caller do it, because every
        caller of this is a safety gate: `level` refuses to set a datum
        without it, and homing refuses to finish without `level`. A part that
        needs one register write to come back should not cost a whole run.

        The recovery is counted and named in the returned reason -- a part
        that needs reviving every time is a failing part, and that has to stay
        visible rather than being silently papered over.
        """
        if not self.have_accel:
            return (False, self.accel_note)

        ok, why = self._accel_live_once(samples, wait_ms)
        if ok:
            return (ok, why)

        self.accel_recoveries += 1
        try:
            self._configure_accel()
        except OSError as e:
            return (False, "%s; reconfigure failed (%s)" % (why, e))
        ok2, why2 = self._accel_live_once(samples, wait_ms)
        if ok2:
            return (True, "%s -- RECOVERED by reconfiguring (it had: %s; "
                          "recovery #%d this session)"
                          % (why2, why, self.accel_recoveries))
        return (False, "%s, and reconfiguring did not revive it (%s)"
                       % (why, why2))

    def _accel_live_once(self, samples=16, wait_ms=5):
