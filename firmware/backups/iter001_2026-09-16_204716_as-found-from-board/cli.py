...
class Console:
    ...
    def do_imu(self, a):
        """imu | imu cal | imu watch -- the payload IMU."""
    ...
    def do_level(self, a):
        ...
        d = self._imu
        if not d.have_accel:
            raise ValueError("no accelerometer -- cannot level")
        if not self.platform.enabled:
            self.platform.enable()
            print("(both motors auto-enabled)")
        ...
        if abs(t0) < tol:
            ...
            if abs(k) < 0.2:
                raise ValueError(
                    ...
                    % k)
            if abs(abs(k) - 1.0) > 0.25:
                print("  NOTE: expected about 1.0. %.3f suggests "
                      "DIFFERENTIAL_N or the microstep setting disagrees "
                      "with the mechanism." % k)
