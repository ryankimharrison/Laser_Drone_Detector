...
#: Departure from 1 g that counts as an impact. A settled payload reads 1 g at
#: any attitude, so this is pure acceleration. An A4988 raises no fault when a
#: stepper skips against a hard stop, and the step counter keeps counting, so
#: the accelerometer is the ONLY witness that a collision happened at all.
LEVEL_IMPACT_G = 0.35
        ...
        if abs(t0) < tol:
            ...
            if k < 0.0:
                raise ValueError(
                    ...
                    % (k, probe))
            # The bound that matters is CONVERGENCE, not closeness to 1.0, and
            # it is not symmetric. The step is -t/k against a true gain of
            # about 1.0, so the tilt left after one iteration is t*(1 - 1/k):
            #
            ...
            #                  exceeds the error and it RUNS AWAY -- 20 deg at
            #                  a time, into a stop.
            #
            # So refuse below 0.5 and merely report above. Banding
            # symmetrically at +/-0.25 (my first attempt at this guard) blocked
            # a perfectly safe k = 1.357 while adding nothing: the crash came
            # from the sign, which is caught above.
            if k < 0.5:
                raise ValueError(
                    "sensitivity %.3f deg/deg is too LOW to drive on. "
                    "Refusing to level: the correction is -t/k, so a k below "
                    "about half the true gain overshoots by more than the "
                    "error and diverges in 20 deg jumps. Check that the "
                    "payload is clear of its stops and that the belt is not "
                    "slipping." % k)
            if k > 1.5:
            ...
            for i in range(10):
                ...
                # (b) DID THE LAST MOVE HELP? If the tilt grew, the direction
                # is wrong whatever k said. One noise-width of slack so a
                # converged loop does not trip on its own jitter.
                if prev is not None and abs(t) > prev + LEVEL_NOISE_DEG:
                    raise ValueError(
                        "tilt got WORSE: %.2f -> %.2f deg after commanding "
                        "%+.2f. The correction is driving away from level, so "
                        "stopping here rather than walking into a stop. "
                        "Sensitivity measured %+.3f deg/deg; if that sign is "
                        "wrong for this rig, pitch is inverted."
                        % (prev, abs(t), step, k))
                prev = abs(t)
                ...
                for _ in range(8):
                    ...
                    if abs(mag - 1.0) > worst:
                        worst = abs(mag - 1.0)
                    time.sleep_ms(10)
                if worst > LEVEL_IMPACT_G:
                    raise ValueError(
                        "|a| departed %.2f g from gravity right after a %+.2f "
                        "deg move -- that is an impact or a platform still "
                        "ringing, not a settled payload. Stopping before the "
                        "next move." % (worst, step))
            t = settle_tilt()
