...
class WristRenderer:
    ...
    def __init__(self, size: Tuple[int, int] = (430, 300), mesh_path: Path = MESH_PATH,
        ...
        self.az = 38.0
        self.el = 16.0
        # Back-face culling roughly halves the fragment count (205 ms -> 111 ms
        # at 430x300). It is only sound because `export_wrist_mesh.py` welds
        # each part before decimating and then runs `fix_normals`, leaving
        # every body consistently wound and positively oriented. Shading stays
        # two-sided anyway, since the bodies are not all closed.
        self.cull = True
    ...
    def _pose(self, pitch: float, yaw: float) -> dict:
        ...
        out = {
            "base": np.eye(3),
            "carrier": Rp,
            "head": Rp @ _rot(self.yaw_axis, yaw),
    ...
    def render(self, pitch: float, yaw: float, *, beam: float = 0.0,
        ...
        self._hud(pil, pitch, yaw, label, sub)
        return pil
    ...
    def _hud(self, pil: Image.Image, pitch, yaw, label, sub):
        ...
        if sub:
            d.text((w - 7 - d.textlength(sub, font=self._font_s), 5), sub,
                   font=self._font_s, fill=HUD_DIM)

        th = BELT_N * (pitch + yaw)
        tb = BELT_N * (pitch - yaw)
        rows = (("pitch", f"{pitch:+7.2f}°"), ("yaw", f"{yaw:+7.2f}°"),
                ("θA", f"{th:+7.1f}°"), ("θB", f"{tb:+7.1f}°"))
        y = h - 4 - len(rows) * 14
        d.rectangle([0, y - 6, 104, h - 1], fill=(12, 16, 22, 190))
        for k, v in rows:
            d.text((8, y), k, font=self._font_s, fill=HUD_DIM)
            d.text((36, y), v, font=self._font_s, fill=HUD_FG)
            y += 14
...
class WristView(tk.Frame):
    ...
    **Quality adapts.** While the pose is moving it renders at 1x, because the
    ...
    def __init__(self, parent, *, size=(400, 280), fps: int = 24,
        ...
        self._pose = (0.0, 0.0, 0.0)          # pitch, yaw, beam
    ...
    # -- feeding ---------------------------------------------------------
    def set_pose(self, pitch_deg: float, yaw_deg: float, beam: float = 0.0) -> None:
